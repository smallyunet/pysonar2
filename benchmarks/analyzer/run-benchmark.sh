...
if [ ! -f "$project_root/target/pysonar-3.3.2.jar" ]; then
  echo "Build target/pysonar-3.3.2.jar with mvn package first." >&2
  exit 2
fi

if [ -n "$change_file" ]; then
  java -cp "$project_root/target/pysonar-3.3.2.jar" \
